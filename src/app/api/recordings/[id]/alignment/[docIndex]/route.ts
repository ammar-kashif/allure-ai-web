...
export async function GET(
  ...
  const res = await fetch(`${BACKEND_URL}/recordings/${recording.backendId}/alignment/${docIndex}`)
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    return NextResponse.json({ error: err.detail || "Alignment failed" }, { status: res.status })
  }
